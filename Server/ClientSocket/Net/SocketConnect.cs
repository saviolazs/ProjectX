    ...
    public class SocketConnect
    {
                ...
                try
                {
                    if (socket.Poll(5, SelectMode.SelectRead))
                    {
                        ...
                        if (recnum == 4)
                        {
                            ...
                            Console.WriteLine("socket.Poll ok" + recnum + "actionId:" + reader.ActionId + " " + reader.RmId.ToString() + "len" + datalen.ToString());
                            lock (sendList)
                            {
                    ...
                    {
                        Close();
                        Console.WriteLine("SelectError Close Socket");
                        break;
        ...
        public bool Send(byte[] data)
        {
            ...
            if (socket != null)
            {
                ...
                if (!success)
                {
                    Console.WriteLine("asyncSend error close socket");
                    Close();
                    return false;
                }
        ...
        public void Request(byte[] data, SocketPackage package)
        {
            ...
            {
                bool bRet = Send(data);
                Console.WriteLine("Socket Request Id:" + package.ActionId.ToString() + " " + package.MsgId.ToString() + "send:" + bRet.ToString());
            }
